...
Framework7.prototype.plugins.indexedlist = function (app, params) {
    ...
    function initIndexedList(page) {
        var eventsTarget = $(page.container).find('.letter_list');
        ...
        function handleTouchMove(e) {
            ...
            if (target.length > 0 && !target.is('.letter_list li')) return;
        ...
        $(page.container).on('click', '.letter_list li', function (e) {
            var target = $(e.target);
            if (!target.is('li')) target = target.parents('li');
            if (target.length > 0) {
                scrollToLetter(target.eq(0).data('index-letter'));
        ...
        function buildLetters() {
            ...
            pageContent.find('.list-group').each(function () {
                var _letterDiv = $(this).find('.c_scroll_key');
                var _letter = _letterDiv.html().trim().charAt(0).toUpperCase();
                _letterDiv.attr('data-index-letter', _letter);
                lettersHtml += '<li data-index-letter="' + _letter + '" class="letter_btn G-Fsize-14 G-color-333">' + _letter + '</li>';
        ...
        function scrollToLetter(letter) {
            var scrollToEl = pageContent.find('.list-group p[data-index-letter="' + letter + '"]').parent();
